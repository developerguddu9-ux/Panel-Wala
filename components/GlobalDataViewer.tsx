...
const GlobalDataViewer: React.FC<Props> = ({ onBack }) => {
  ...
  const handleDeleteAll = async () => {
    ...
    try {
      ...
      alert("Delete failed: " + (e as any).message);
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  return (
    <div className="h-full flex flex-col bg-gray-50">
       <header className="bg-white px-4 py-3 shadow-sm border-b border-gray-200 flex items-center justify-between sticky top-0 z-20 flex-none">
        <div className="flex items-center space-x-3 overflow-hidden">
            ...
            <div className="overflow-hidden">
              <h1 className="font-bold text-lg text-gray-900 leading-tight">Global Data</h1>
              <p className="text-[10px] text-gray-500 font-bold uppercase tracking-wider">Real-time device info nodes</p>
            </div>
        </div>
        ...
        <button 
          ...
          className="p-3 text-red-500 hover:bg-red-50 rounded-lg disabled:opacity-30 transition-colors shrink-0"
          title="Delete All Data Nodes"
        >
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {/* Search bar below header like SMS Viewer */}
      <div className="bg-white px-4 py-2 border-b border-gray-100 shadow-sm z-10">
      ...
      <div className="flex-1 overflow-auto p-4 md:p-6 scrollbar-hide">
        {loading ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pb-20 max-w-7xl mx-auto">
            {filteredUsers.map(u => (
              <div key={u.userId} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow flex flex-col">
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-fadeIn">
          <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm text-center">
             ...
             <h3 className="text-xl font-bold text-gray-900 mb-2">Clear Global Data?</h3>
             <p className="text-sm text-gray-500 mb-6">This will delete ALL <span className="font-bold text-gray-800">Info</span> and <span className="font-bold text-gray-800">Card</span> nodes for every device. This is irreversible.</p>
             <div className="flex space-x-3">
                ...
                <button 
                  ...
                  disabled={isDeleting}
                  className="flex-1 py-3 bg-red-600 text-white rounded-xl font-bold shadow-lg hover:bg-red-700 disabled:opacity-50 flex items-center justify-center space-x-2"
                >
