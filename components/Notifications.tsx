...
const Notifications: React.FC<Props> = ({ onBack }) => {
  ...
  useEffect(() => {
    // Correctly mapping notifications from the 'notifications' plural node
        ...
        {loading ? (
           ...
           <div className="space-y-4 max-w-3xl mx-auto">
              {notifications.map(n => (
                  <div 
                    key={n.id} 
                    className={`p-5 rounded-xl border transition-all cursor-pointer ${n.read ? 'bg-white border-gray-200 opacity-80' : 'bg-white border-l-4 border-l-app-green border-y-gray-200 border-r-gray-200 shadow-md transform hover:-translate-y-0.5'}`} 
                  ...
                  >
                      ...
                      {/* FIXED BUG: Ensure both title AND text content are displayed correctly */}
