...
import { subscribeToUsers, subscribeToFavorites, updateFavoritesInDb } from '../services/firebase';
...
const FavoritesList: React.FC<Props> = ({ onBack, onSelectUserForSMS, onSelectUserForData, user }) => {
  const [users, setUsers] = useState<DeviceInfo[]>([]);
  const [favorites, setFavorites] = useState<string[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    // Subscribe to favorites from DB
    let unsubFavs = () => {};
    if (user?.userId) {
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6">
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
            {favoriteUsers.map(u => (
              <div key={u.userId} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden flex flex-col">
                <div className={`h-1.5 w-full ${u.Status === 'Online' ? 'bg-green-500' : 'bg-gray-200'}`}></div>
                <div className="p-4 flex-1 flex flex-col">
                   <div className="flex justify-between items-start mb-3">
                     <div className="flex-1 pr-2 min-w-0">
                       <div className="flex items-center space-x-1.5 min-w-0">
                          <h3 className="font-bold text-gray-900 truncate">{u.Brand || 'Unknown'}</h3>
                          {u.checked && <svg className="w-5 h-5 text-blue-500 fill-current flex-shrink-0" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" /></svg>}
                       </div>
                       <p className="text-xs text-gray-500 font-mono mt-1 truncate">{u.DeviceId}</p>
                     </div>
                     <button onClick={() => toggleFavorite(u.userId)} className="p-1.5 text-yellow-400 hover:text-gray-300">
                   ...
                   <div className="mt-auto grid grid-cols-2 gap-3 pt-4">
                      <button onClick={() => onSelectUserForSMS(u)} className="py-2 bg-gray-50 hover:bg-app-green hover:text-white text-gray-600 rounded-lg text-sm font-semibold transition-colors">SMS</button>
                      <button onClick={() => onSelectUserForData(u)} className="py-2 bg-gray-50 hover:bg-blue-600 hover:text-white text-gray-600 rounded-lg text-sm font-semibold transition-colors">Data</button>
                   </div>
                </div>
              </div>
            ))}
